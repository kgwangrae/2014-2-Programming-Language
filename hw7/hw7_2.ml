 ...
 * Low Fat M: static Polymorphic type checker
...
module M_PolyChecker : M_PolyChecker = struct
  ...
  | ForAll of freevars * typevar
  and freevars = typevar list (* List of 'unknowns' *) 
  ...
  let rec ty_equal : typevar -> typevar -> bool = fun ty1 ty2 ->
    ...
    | (_, _) -> false (* TODO : ForAll : equality ? *)
  ...
  let rec rm_dup = fun lst ->
    ...
    | [] -> []
  let rec rm_lst_from = fun target src ->
    match src with
    | (k,v)::tl -> if (List.exists (fun x -> ty_equal x k) target) 
                then (rm_lst_from target tl) else (k,v)::(rm_lst_from target tl)
    | [] -> []
  ...
  let rec subst_apply_one : subst -> typevar -> typevar = fun subst ty ->
    ...
    | ForAll (ftvs, ty_in) ->
        ForAll (ftvs, subst_apply_one subst ty_in)
  ...
  let rec contain : typevar -> typevar -> bool = fun target src ->
    ...
    | Oneof (_, tylst) -> (contain_lst tylst)
    | ForAll (ftvs, ty_in) -> (contain_lst ftvs) || (contain target ty_in)

  (* Poly functions *)
  let instantiate : freevars -> subst = fun ftvs ->
    List.map (fun elem -> (elem, get_new_var())) ftvs
  
  let rec get_ftv_typ : typevar -> freevars = fun tyvar ->
    let result = (
    match tyvar with
    | Int | Bool | String -> []
    | Pair (l, r) -> (get_ftv_typ l)@(get_ftv_typ r)
    | Loc (l) -> (get_ftv_typ l)
    | Fun (l, r) -> (get_ftv_typ l)@(get_ftv_typ r)
    | Unknown _ -> [tyvar]
    | Oneof (_, tylst) -> (
        List.fold_left (fun acc curr -> (get_ftv_typ curr)@acc) [] tylst)
    | ForAll(free_in, ty_in) ->
        let result_in = get_ftv_typ ty_in in 
        List.fold_left 
        (fun acc curr -> (if (List.mem curr free_in) then acc else curr::acc))
        [] result_in
    )
    in
    rm_dup result
  
  let rec get_ftv_env : typeenv -> freevars = fun tyenv ->
    rm_dup (List.fold_left (fun acc (id,typ) -> (get_ftv_typ typ)@acc) [] tyenv)
  
  let rec get_ftv : typevar -> typeenv -> freevars = fun tyvar tyenv ->
    let rm_target = get_ftv_env tyenv in
    rm_dup(
      List.fold_left 
      (fun acc curr -> (if (List.mem curr rm_target) then acc else curr::acc))
      [] (get_ftv_typ tyvar)
    )

  let generalize : typeenv -> typevar -> typevar = fun tyenv tyvar ->
    let result = get_ftv tyvar tyenv in
    if (0 = List.length result) then tyvar else ForAll (result, tyvar)

  (* Conservatively determines whether the pgm (may) allocate any memory block 
   * NOT to save generalized type variable into memory *)
  let rec expansive : exp -> bool = fun pgm -> 
    match pgm with
    | CONST const -> false
    | VAR id -> false
    | FN (id, exp) -> false (* Function def itself doesn't allocate any memory*)
    | APP (e1, e2) -> true
    | LET (decl, e2) ->
       (match decl with
        | NREC (id, e1) -> (expansive e1) || (expansive e2)
        | REC (id, e1) -> (expansive e1) || (expansive e2))
    | IF (e1, e2, e3) -> (expansive e1) || (expansive e2) || (expansive e3)
    | BOP (bop, e1, e2) -> (expansive e1) || (expansive e2)
    | READ -> false
    | WRITE exp -> expansive exp
    | MALLOC exp -> true       
    | ASSIGN (e1, e2) -> (expansive e1) || (expansive e2)
    | BANG exp -> expansive exp
    | SEQ (e1, e2) -> (expansive e1) || (expansive e2)     
    | PAIR (e1, e2) -> (expansive e1) || (expansive e2)
    | SEL1 exp -> (expansive exp)         
    | SEL2 exp -> (expansive exp)        
  ...
  let rec m_algo : (typeenv * exp * typevar) -> subst = fun (env, exp, tyvar) -> 
    ...
    | VAR id -> (
        try (
          let lookup = (get env id (fun x y -> x = y))
          in
          match lookup with
          | ForAll (free_in, ty_in) -> 
              unify (subst_apply_one (instantiate free_in) ty_in) tyvar
          | _ -> unify lookup tyvar
        )
    ...
    | LET (decl, e_in) -> (
      ...
      | REC (id, e_decl) -> ( (*Recursive Functions*)
        let is_safe = not (expansive e_decl) in
        let new_var = get_new_var () in
        let new_var2 = get_new_var () in
        let id_ty = Fun (new_var, new_var2) in
        let env_self = env_add env id 
                       (if (is_safe) then (generalize env id_ty)
                        else id_ty) in
        let s1 = m_algo (env_self, e_decl, id_ty) in
        let env_s1 = subst_apply_env s1 env in
        let s2 = m_algo (
          (env_add env_s1 id (
            if (is_safe) then (generalize env_s1 (subst_apply_one s1 id_ty))
            else (subst_apply_one s1 id_ty)
          )),
          e_in, 
      ...
      | NREC (id, e_decl) -> ( (*Any binding*)
        ...
        let s2 = m_algo (
          (env_add env_s1 id 
           (if (is_safe) then (generalize env_s1 (subst_apply_one s1 new_var))
                         else (subst_apply_one s1 new_var))), 
          e_in, 
          subst_apply_one s1 tyvar
        ) 
        in
  ...
  let rec typevar_to_types : typevar -> types = fun tv ->
    ...
    | ForAll (_, _) -> tyerr "Polymorphic result not allowed"
